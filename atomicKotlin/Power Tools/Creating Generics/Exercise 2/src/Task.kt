...
class CrateList<T> : ArrayList<Crate<T>>() {
    private class DecoratedCrate<T>(val contents: T) : Crate<T>(contents) {
...
fun main() {
    ...
    intCrates eq "[[0], [1], [2], [3], [4], [5], [6], [7]]"
    val stringCrates = CrateList<String>()
    ('a'..'h').map { it.toString() }.forEach { stringCrates.add(it) }
    stringCrates eq "[[a], [b], [c], [d], [e], [f], [g], [h]]"
