...
class City(val name: String) {
    ...
    override fun toString() = "($name, " + "connections: ${connections.map { it.name }})"
}
...
fun City.allReachableRecursive(): Set<City> {
    ...
    fun addConnectionsFrom(current: City) {
        ...
        current.connections.forEach { city ->
            if (!connections.contains(city)) addConnectionsFrom(city)
        }
    }
...
fun main() {
    ...
    liverpool.allReachableRecursive() eq setOf(liverpool, manchester, leeds)
    dublin.allReachableRecursive() eq setOf(dublin)

    liverpool.allReachableIterative() eq setOf(liverpool, manchester, leeds)
    dublin.allReachableIterative() eq setOf(dublin)
}